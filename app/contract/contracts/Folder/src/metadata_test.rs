...
use crate::{
    ...
    types::{
        ContractHealth, DeploymentMetadata, FeatureFlags, SchemaCompatibility, SupportedVersions,
        UpgradeState,
    },
...
fn metadata_schema_compatibility_matches_current_versions() {
    ...
    let compat: SchemaCompatibility =
        client.check_schema_compatibility(&CURRENT_CONTRACT_VERSION, &EVENT_SCHEMA_VERSION);
...
fn metadata_schema_compatibility_rejects_unknown_versions() {
    ...
    let compat: SchemaCompatibility = client
        .check_schema_compatibility(&(CURRENT_CONTRACT_VERSION + 1), &(EVENT_SCHEMA_VERSION + 1));
    assert!(!compat.contract_compatible);
    assert!(!compat.event_compatible);
    assert!(!compat.overall_compatible);
}
